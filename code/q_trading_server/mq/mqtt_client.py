...
class MqttClient(MqBaseImpl):
    ...
    # 保活间隔（秒），低于 broker/网络设备的空闲超时（~30s），避免被主动断开
    KEEPALIVE: int = 15
        ...
        if self.mqtt_need_tls:
            try:
                client.tls_set(
                    ...
                    certfile=self.mqtt_cert_file,
                    keyfile=self.mqtt_key_file,
    ...
    def _on_connect(
        ...
        if rc == 0:
            ...
            self.logger.error(
                "MQTT 连接失败，返回码: %s (0=成功, 1=协议版本错误, 2=client_id 拒绝, 3=服务器不可用, 4=用户名密码错误, 5=未授权)",
                rc,
            )
    ...
    def _on_disconnect(
        ...
        :param client: MQTT 客户端实例
        :param userdata: 用户数据
        :param rc: 断开原因码，paho-mqtt MQTT_ERR_* 常量
        """
        self._is_connected = False
        if rc != 0:
    ...
    def subscribe(
        ...
        try:
            ...
            self.logger.info("MQTT 订阅 topic 成功: %s (QoS: %d)", topic, qos)
            return True
        except Exception as err:
