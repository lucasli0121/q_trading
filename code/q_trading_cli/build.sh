...
# ---- 配置（可按需修改） ----
IMAGE_NAME="${IMAGE_NAME:-q_trading_cli}"
...
clean() {
    log_step "清理构建产物..."
    # 容错清理：dist 中可能存在容器生成的 root 属主文件（权限不足无法删除），
    # 对 docker 构建无影响，忽略删除失败以免因 set -e 中断整个构建。
...
package_source() {
    local version="$1"
    local tar_file="q_trading_cli-${version}-src.tar.gz"
    # 输出到上级目录避免 tar 自身被包含
...
build_pyinstaller() {
    local version="$1"
    local dist_dir="dist/q_trading_cli-${version}"
    local tar_file="q_trading_cli-${version}-bin.tar.gz"
...
HOOKEOF
    ...
    pyinstaller \
        ...
        --noconfirm --clean \
        q_trading_cli.spec 2>&1 | tail -20

    if [[ ! -d "${dist_dir}/q_trading_cli" ]]; then
        log_error "PyInstaller 打包失败"
        exit 1
    fi

    log_info "可执行文件: ${dist_dir}/q_trading_cli/q_trading_cli"

    # 打包为 tar.gz
    log_step "压缩二进制包: ${tar_file}"
    (
        cd "${dist_dir}"
        tar -czf "/tmp/${tar_file}" q_trading_cli/
    )
...
package_tar() {
    local version="$1"
    local tar_file="q_trading_cli-${version}.tar"
    local full_image="${IMAGE_NAME}:${version}"
...
show_info() {
    ...
    echo ""
    echo "运行容器:"
    echo "  docker run -d -p 8085:8085 --name q_trading_cli ${full_image}"
    echo ""
