...
class UserApi:
    ...
    def set_online_status(self, is_online: bool) -> str:
        """设置用户在线状态 — 在线时自动记录上线时间，离线时清空。

        :param is_online: 是否在线
        :return: 操作结果提示信息
        """
        body: dict[str, bool] = {"is_online": is_online}
        result: Any = self._client.post("/api/user/online", body)
        return str(result)
